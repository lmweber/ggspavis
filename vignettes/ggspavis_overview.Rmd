...
First we start with a quick demo of the Visium mouse brain data. We perform the visualization of library size and gene expression level that can be easily derived. Both `plotVisium()` and `plotSpots()` reflects the spatial coordinates of spots, with the former overlays spots on the H&E image. Note that `plotVisium()` can only take a `SpatialExperiment` class object, while other functions in the package can take both `SpatialExperiment` or `SingleCellExperiment` class object. 
...
plotVisium(spe, annotate = "Gapdh", highlight = "in_tissue") | 
  plotVisium(spe, annotate = "Mbp", highlight = "in_tissue")
```

Two other cases of `plotVisium()` is to show only spots or only the H\&E image. Note that `plotVisium(image = FALSE)` can be used as `plotSpots()`, but it also includes spots that are not in tissue.

```{r, fig.width=8, fig.height=3.5}
plotVisium(spe, annotate = "Mbp", highlight = "in_tissue", image = FALSE) | 
  plotVisium(spe, annotate = "Mbp", highlight = "in_tissue", spots = FALSE)
```

For `plotSpots()` for Visium, we by default subset to spots that are in tissue. Palettes in `plotSpots()` can be changed like in `plotVisium()`. 
...
We can compare the ground truth annotation with clustering result with `plotSpots()` (or `plotVisium()` overlaying H&E). Here we also demonstrate the on-plot text annotation functionality with `plotSpots(text_by = )`. 

```{r, fig.width=8, fig.height=4.5}
spe$Cluster <- factor(spe$Cluster)
plotSpots(spe, annotate = "layer_guess_reordered") |
  plotSpots(spe, annotate = "Cluster", text_by = "Cluster")
```
...
Another example with `plotDimRed(type = "PCA")` on ground truth annotation, with on-plot text annotation on each cluster, and another highly variable gene.
...
We also demonstrate the `zoom` to only spots area and `show_axis` on labels and ticks functionality in `plotVisium()`, which are both set to default. # TODO: for Visium in preprint. 

```{r}
  plotVisium(spe, annotate = "ground_truth", highlight = "in_tissue", palette = "libd_layer_colors", zoom = TRUE, show_axis = TRUE)
```
